 ...
 if (obj.hasOwnProperty(prop) && (overwrite !== false || !target.hasOwnProperty(prop))) { target[prop] = obj[prop] }
    ...
    return function (str, direction) {
      ...
      for (var i$7 = 0; i$7 < len;) {
        if (countsAsLeft.test(types[i$7])) {
          ...
          var pos = i$7; var at = order.length; var isRTL = direction == 'rtl' ? 1 : 0
  ...
  function off (emitter, type, f) {
    if (emitter.removeEventListener) {
      ...
      var map = emitter._handlers; var arr = map && map[type]
      if (arr) {
 ...
 for (var i = 0; i < lineView.rest.length; i++) { insertLineWidgetsFor(cm, lineView.rest[i], lineView, dims, false) }
  ...
  function insertLineWidgetsFor (cm, line, lineView, dims, allowAbove) {
    ...
    for (var i = 0, ws = line.widgets; i < ws.length; ++i) {
      var widget = ws[i]; var node = elt('div', [widget.node], 'CodeMirror-linewidget' + (widget.className ? ' ' + widget.className : ''))
...
bottom: rect.bottom * scaleY }
 ...
 for (var i = 0; i < line.widgets.length; i++) {
  ...
  function drawSelectionRange (cm, range, output) {
    ...
    var sFrom = range.from(); var sTo = range.to()
 ...
 for (var i = 0; i < line.widgets.length; ++i) {
  ...
  function resolveScrollToPos (cm) {
    ...
    if (range) {
      ...
      var from = estimateCoords(cm, range.from); var to = estimateCoords(cm, range.to)
 ...
 for (var j = 0; j < align.length; j++) { align[j].style.left = left }
  ...
  Selection.prototype.equals = function (other) {
    ...
    for (var i = 0; i < this.ranges.length; i++) {
      var here = this.ranges[i]; var there = other.ranges[i]
...
to: to,
 ...
 for (var i$1 = 0; i$1 < this.lines.length; ++i$1) {
      var visual = visualLine(this.lines[i$1]); var len = lineLength(visual)
...
full: true }, true)
    ...
    getCursor: function (start) {
      var range = this.sel.primary(); var pos
...
shared: options && options.shared,
 ...
 for (var i = 0; i < spans.length; ++i) {
    ...
    findMarks: function (from, to, filter) {
      ...
      var found = []; var lineNo = from.line
 ...
 for (var i = 0; i < sps.length; ++i) { if (sps[i].from != null) { markers.push(sps[i].marker) } }
    ...
    posFromIndex: function (off) {
      var ch; var lineNo = this.first; var sepSize = this.lineSeparator().length
      this.iter(function (line) {
...
moved: false,
  ...
  function applyTextInput (cm, inserted, deleted, sel, origin) {
    ...
    for (var i$1 = sel.ranges.length - 1; i$1 >= 0; i$1--) {
      ...
      var from = range.from(); var to = range.to()
      if (range.empty()) {
...
opaque: options && options.opaque,
 ...
 for (;;) {
      ...
      cursorCoords: function (start, mode) {
        var pos; var range = this.doc.sel.primary()
 ...
 for (var i = 0; i < doc.sel.ranges.length; i++) { doc.sel.ranges[i].goalColumn = goals[i] }
      ...
      getScrollInfo: function () {
        ...
        return { left: scroller.scrollLeft,
  ...
  function domTextBetween (cm, from, to, fromLine, toLine) {
    ...
    function walk (node) {
      if (node.nodeType == 1) {
        ...
        var markerID = node.getAttribute('cm-marker'); var range
        if (markerID) {
